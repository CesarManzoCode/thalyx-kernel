...
| `control-lost` | escenario 5, `AFTER_PREPARE` / `LOSE_CONTROL` | El auditor deja de leer y el plano de control se llena con una preparación durable. |
...
- **`control-lost` para al auditor a propósito, y llena el plano donde la directiva dice.** Es andamiaje y está marcado como tal: el supervisor deja de leer porque el escenario se lo pide, y el servicio llena las celdas ordinarias que quedan en `AFTER_PREPARE` como cualquier otro corte, sin perder nada (el llenado se detiene en el espacio que el plano reporta, no en el rechazo que contaría una pérdida). Lo que muestra es la respuesta del mecanismo a un plano lleno con una preparación viva —la admisión cubierta rechazada, el servicio diciéndose `UNAVAILABLE`, el llamante recibiendo `UNKNOWN`, nada en el medio sin reclamar—, no que un auditor real fuera a pararse. Antes el supervisor llenaba el plano desde fuera mientras los clientes lo consumían, y qué admisión encontraba el plano lleno era una carrera: en `perf/ipc-scalability` el caso mostró cuatro cosas distintas en cuatro ejecuciones (un plano que no se llenaba, un llamante rechazado antes de preparar nada, un commit en el medio cuyo flush fue la admisión rechazada, una nota del servicio contada como recibo perdido) hasta que el llenado pasó a ser una directiva en un punto.
