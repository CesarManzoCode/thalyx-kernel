...
| Puerta K5 | PASS en 34 criterios decididos por separado sobre cuatro ejecuciones —`smoke`, `surface`, `work`, `engine`—, desde los registros del kernel y los bytes del medio; la comparación con Linux es ejecución del anfitrión etiquetada como tal. [Detalle y límites](../evidence/k5-thalyx-port.md). |
| Autocomprobación de la puerta K5 | 52 ejecuciones dañadas de una forma cada una, las 52 detectadas por el criterio que les corresponde. |
...
**Último hito terminado y pusheado:** **K5 etapas `smoke`, `surface`, `work` y `engine`**, en la rama `feat/k5-thalyx-port-tools`. K4 quedó completo en `feat/k4-durable-managed-state`.

**Qué está verde, medido en este árbol y con el mismo binario de kernel en las cinco fases:** puerta K1 13/13, puerta K2 21/21 con autocomprobación 28/28, puerta K3 28/28 con autocomprobación 57/57, puerta K4 31/31 con autocomprobación 48/48, formato K4 12/12, puerta K5 34/34 con autocomprobación 52/52, protocolos K5 PASS, ABI 4/4, vault 45 notas PASS, modelos PASS, `fmt` limpio.
...
**Qué demostró exactamente K5 hasta aquí:** un programa C compilado por una toolchain cruzada del anfitrión para un target propio, cargado por el kernel, activado, planificado, con coma flotante en hardware que el anfitrión recalcula y exige idéntica, y un heap que crece por objetos de memoria hasta que el ámbito lo rechaza; la vertical semántica de Thalyx entera sobre el estado administrado de K4; QuickJS real ejecutando un programa que sale de la versión publicada, alcanzando el workspace solo por llamadas mediadas; una herramienta nativa real que compila el candidato sellado en un dominio propio, ejecuta sus aserciones y decide la publicación, con lo que costó leído de la contabilidad del kernel; y un motor CPU residente real —llama.cpp entero, en C++, con la biblioteca estándar de C++ enlazada y su clausura en `user/native`— que carga los pesos una vez, sirve dos peticiones de un programa, cobra cada inferencia al ámbito de quien preguntó y la residencia al suyo, y produce byte a byte las completaciones que el motor de Thalyx da en Linux.

**Qué no demostró todavía:** no hay dos trabajos rivales contra el mismo motor, ni cancelación de un trabajo mientras el motor residente calcula para él, ni un corte en publicación dentro de esta vertical —EXP-10 pide los tres—; la comparación con Linux cubre el motor y no declara todavía los perfiles de los dos backends ni un rechazo de perfil insuficiente —la parte K5 de EXP-11—; no hay comprobación de tipos ni `cargo` dentro del kernel. Los límites de K3 y K4 se heredan enteros.

**Siguiente paso exacto al reanudar:** la matriz de casos adversarios de EXP-10 sobre la etapa `engine` —dos trabajos rivales, cancelación durante el servicio residente, corte y recuperación en publicación—, y la parte K5 de EXP-11. [La ruta](phases.md).

## Siguiente trabajo: terminar K5

Según [la ruta](phases.md), las cuatro etapas están ejecutadas; queda la matriz adversaria de EXP-10 y la parte K5 de EXP-11. Lo que K4 deja abierto y K5 hereda: el perfil de durabilidad declara que la supresión la hace el driver del invitado, y cualquier afirmación más fuerte necesita conocer el comportamiento de flush de un dispositivo real —el mismo tipo de dependencia que el perfil de DMA declara sobre la unidad de remapeo—.
