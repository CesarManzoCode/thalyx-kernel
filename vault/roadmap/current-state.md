...
**2026-09-10 · Fundación 0.1.0 · K0, K1, K2, K3, K4 y K5 completos. K6 pendiente.**

> **Punto de reanudación.** Este documento es el checkpoint. La sección [Reanudar aquí](#reanudar-aquí) dice exactamente dónde empieza el trabajo siguiente; no hace falta reauditar K0, K1, K2, K3, K4 ni K5.
...
Existe además un kernel que arranca, un sistema de capacidades que se ejecuta sobre él, una ejecución en cuatro procesadores con un dispositivo de bloque real conducido desde usuario, un almacén de versiones que sobrevive a la ejecución que lo escribió, y la semántica de Thalyx corriendo sobre todo eso: un target nativo propio, una libc escrita aquí, QuickJS real, una herramienta real y llama.cpp entero como motor residente, con el kernel cobrando cada inferencia a quien la pidió. El workspace tiene loader UEFI, protocolo de arranque, kernel con `arch/x86_64`, interfaz V0 generada desde un esquema, quince programas de usuario en Rust y cuatro en C y C++ sobre un target nativo propio, tres bibliotecas que comparten —el runtime, el formato durable y el paquete K5— y una libc escrita aquí, con toolchain fijada y construcción reproducible desde un solo script para las cinco fases.
...
- **Thalyx entero sobre este kernel.** K5 ejecuta la semántica de Thalyx —versión, contexto, programa, herramienta, motor, publicación, evidencia— con sus mecanismos: QuickJS real, llama.cpp real, el servicio de estado de K4. No ejecuta el binario de Thalyx: no hay `cargo` ni compilador de Rust dentro del kernel, no hay comprobación de tipos, el modelo es el que Thalyx usa para ejercitar su propio motor y no uno de trabajo, hay un hilo de cómputo, y el perfil de Linux es una lectura y no una ejecución. Rendimiento y prueba formal general siguen sin medir ni existir.
...
**Último hito terminado y pusheado:** **K5 completo** —las cuatro etapas, la matriz EXP-10 y la parte K5 de EXP-11— en la rama `feat/k5-thalyx-port-tools`. K4 quedó completo en `feat/k4-durable-managed-state`.

**Qué está verde, medido en este árbol y con el mismo binario de kernel en las cinco fases:** puerta K1 13/13, puerta K2 21/21 con autocomprobación 28/28, puerta K3 28/28 con autocomprobación 57/57, puerta K4 31/31 con autocomprobación 48/48, formato K4 12/12, puerta K5 47/47 con autocomprobación 87/87, protocolos K5 PASS, ABI 4/4, vault 45 notas PASS, modelos PASS, `fmt` limpio.
...
**Qué demostró exactamente K5:** un programa C compilado por una toolchain cruzada del anfitrión para un target propio, cargado por el kernel, activado, planificado, con coma flotante en hardware que el anfitrión recalcula y exige idéntica, y un heap que crece por objetos de memoria hasta que el ámbito lo rechaza; la vertical semántica de Thalyx entera sobre el estado administrado de K4; QuickJS real ejecutando un programa que sale de la versión publicada, alcanzando el workspace solo por llamadas mediadas; una herramienta nativa real que compila el candidato sellado en un dominio propio, ejecuta sus aserciones y decide la publicación, con lo que costó leído de la contabilidad del kernel; y un motor CPU residente real —llama.cpp entero, en C++, con la biblioteca estándar de C++ enlazada y su clausura en `user/native`— que carga los pesos una vez, sirve dos peticiones de un programa, cobra cada inferencia al ámbito de quien preguntó y la residencia al suyo, y produce byte a byte las completaciones que el motor de Thalyx da en Linux; y esa misma vertical bajo adversidad: dos trabajos rivales sobre un motor sin mezclar prompts, respuestas ni autoridad, un trabajo cerrado mientras el motor calcula para él con el motor intacto para el siguiente, y tres publicaciones que fallan y se recuperan en el arranque siguiente; y el perfil del backend declarado en cada versión, contrastado rasgo a rasgo con los registros del kernel y del motor, con la comprobación que no tiene rechazada por su nombre y las diferencias con Linux escritas en la misma fixture.

**Qué no demostró:** no hay comprobación de tipos ni `cargo` dentro del kernel —el perfil lo declara y pedirlo se rechaza—; el modelo es pequeño y hay un hilo de cómputo, así que nada aquí es una medida de rendimiento; el perfil de Linux es una lectura estática y lo único de Linux que se ejecuta es el motor de referencia en el anfitrión, etiquetado como tal. Los límites de K3 y K4 se heredan enteros.

**Siguiente paso exacto al reanudar:** K6 —protocolos emparejados sobre los dos backends, medidas de contención, IPC, memoria, colas y recuperación con controles, revisión de los parámetros V0, hardware físico acotado—. K6 empieza con la comparación de EXP-11 que K5 dejó preparada: la misma pregunta, de una sola fuente, a dos backends con perfiles declarados. [La ruta](phases.md).

## Siguiente trabajo: K6

Según [la ruta](phases.md), K5 está ejecutado entero en su alcance y K6 es comparación, endurecimiento y primera referencia. Lo que K4 deja abierto y K5 hereda: el perfil de durabilidad declara que la supresión la hace el driver del invitado, y cualquier afirmación más fuerte necesita conocer el comportamiento de flush de un dispositivo real —el mismo tipo de dependencia que el perfil de DMA declara sobre la unidad de remapeo—.
