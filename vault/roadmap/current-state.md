...
- **Escalabilidad del IPC.** Un solo cerrojo serializa cada entrada al kernel que toca un objeto. K6 lo midió: con un par de llamante y contestador el nativo hace 2 352 idas y vueltas por corte de 10 ms y Linux 1 516; con cuatro pares, 1 806 contra 11 912. Un cerrojo de turno quitó la inanición, no la serialización. Partirlo es un rediseño con su propia evidencia, y no se hizo.
...
**Siguiente paso exacto al reanudar:** no hay una fase K7 definida en [la ruta](phases.md). Lo que la evidencia de K6 pone primero, en orden de cuánto pesa: partir el cerrojo de la máquina con evidencia propia ([OQ-04](open-questions.md)); arrancar en la máquina inventariada ([OQ-05](open-questions.md)); y diagnosticar la cola del cierre.

## Siguiente trabajo

Según [la ruta](phases.md), las seis fases están ejecutadas en su alcance y este árbol es la primera referencia sobre KVM. Lo que K4 deja abierto y K5 y K6 heredan —el perfil de durabilidad declara que la supresión la hace el driver del invitado, y el de DMA que no hay unidad de remapeo programada— necesita hardware físico, que está inventariado y no arrancado. Lo que K6 deja medido y sin mover es el cerrojo único de la máquina.
