...
**Estado: constitución técnica 0.1.0; K1, K2, K3 y K4 completados.** Un mismo binario de kernel arranca en QEMU y sostiene cuatro paquetes distintos: dominios en ring 3 preemptados por timer, un sistema de capacidades con autoridad y trabajo atribuible, cuatro procesadores con un driver virtio-blk conducido desde usuario, y un servicio de estado que publica versiones inmutables sobre un medio que persiste entre arranques y se recupera de cortes en cada punto de escritura.

Lo que **no** existe: aislamiento de DMA —no hay unidad de remapeo programada—, durabilidad frente a un corte de energía, hardware físico, Thalyx sobre este kernel, y cualquier medida de rendimiento. Cada nota de evidencia declara sus propios límites en lugar de dejarlos implícitos. Los modelos de investigación incluidos no son una implementación del kernel ni una prueba general de sus propiedades.
...
- [Ver qué ejecutó K1 y qué no demuestra](vault/evidence/k1-protected-boot.md), y lo mismo para [K2](vault/evidence/k2-objects-authority-work.md), [K3](vault/evidence/k3-smp-devices.md) y [K4](vault/evidence/k4-durable-state.md).
