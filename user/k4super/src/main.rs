...
fn run() -> ! {
    ...
    loop {
        ...
        //
        // The log fills where the case's directive says, inside the service,
        // with a preparation durable: filled from here, against clients that
        // were consuming it at the same time, which admission met the full
        // log was a different one each run. This loop only stops reading.
        //
